...
class InferenceGraphDataset():
    def __init__(self, np_file_path, device, encoder, training_graph):
        ...
        self.data_id["head"] = np.array([[self.training_graph.node2id[head],
                                          self.training_graph.node2id[tail], hid] for head, tail, hid in
                                         self.data["head"] \
                                         if len(head.split()) < MAX_NODE_LENGTH and len(
                tail.split()) < MAX_NODE_LENGTH])  # if (self.training_graph.node2id[head], self.training_graph.node2id[tail]) not in train_edges
...
class InferenceSimpleDataset():
    def __init__(self, np_file_path, device, encoder, training_graph):
        ...
        self.data_id["head"] = np.array([[self.training_graph.node2id[head],
                                         ...
                                         self.data["head"] \
                                         if
                                         len(head.split()) < MAX_NODE_LENGTH and len(tail.split()) < MAX_NODE_LENGTH])
        self.data_id["tail"] = np.array([[self.training_graph.node2id[head],
                                          self.training_graph.node2id[tail], -1] for head, tail in self.data["tail"] \
                                         if
                                         len(head.split()) < MAX_NODE_LENGTH and len(tail.split()) < MAX_NODE_LENGTH])
...
class GraphDataset():
    ...
    def __init__(self, nx_file_path,
        ...
        self.train_id, self.val_id, self.test_id = [int(s * len(ATOMIC_edges)) for s in
                                                    np.cumsum(split) / np.sum(split)]
        ...
        if random_split:
            train_edges_pos_all, val_edges_pos_all, test_edges_pos_all = self.get_random_split(current_edge_list,
                                                                                               edge_by_htid)
        else:
            train_edges_pos_all, val_edges_pos_all, test_edges_pos_all = self.get_split_from_atomic(edge_by_htid)

        print('Number of positive training examples:{}, validating:{}, testing:{}'.format(len(train_edges_pos_all),
                                                                                          len(val_edges_pos_all),
                                                                                          len(test_edges_pos_all)))

        if len(train_edges_pos_all) > max_train_num:
            train_edges_pos = list(
                np.array(train_edges_pos_all)[np.random.permutation(len(train_edges_pos_all))[:max_train_num]])
            val_edges_pos = list(np.array(val_edges_pos_all)[np.random.permutation(len(val_edges_pos_all))[
                                                             :int(max_train_num * split[1] / split[0])]])
            test_edges_pos = list(np.array(test_edges_pos_all)[np.random.permutation(len(test_edges_pos_all))[
                                                               :int(max_train_num * split[2] / split[0])]])
        else:
        ...
        print('Number of positive training examples after trucating:{}, validating:{}, testing:{}'.format(
            len(train_edges_pos), len(val_edges_pos), len(test_edges_pos)))
        ...
        if negative_sample == "fix_head":
            ...
            print(
                "num of prepared neg for train:{}, dev:{}, test:{}".format(len(neg_train), len(neg_val), len(neg_test)))
            neg_edges = []
            num_neg = len(train_edges_pos) + len(val_edges_pos) + len(test_edges_pos)
        ...
        val_edges_neg = neg_edges[int(len(train_edges_pos) * neg_prop):int(
            (len(train_edges_pos) + len(val_edges_pos)) * neg_prop)]
        test_edges_neg = neg_edges[int((len(train_edges_pos) + len(val_edges_pos)) * neg_prop):]
        print('Number of negative examples after trucating:{}, validating:{}, testing:{}'.format(len(train_edges_neg),
                                                                                                 len(val_edges_neg),
                                                                                                 len(test_edges_neg)))
        ...
        self.id2nodestoken = dict([(self.node2id[line], torch.tensor(self.tokenizer.encode(line,
            ...
            device)) for line in tqdm(self.node2id)])
